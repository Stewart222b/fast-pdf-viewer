...
- The builder verifies the pinned PDF.js `6.3.289` `VERSION`, `MANIFEST.json`, SHA-256 entries, modern and legacy modules, and the `cmaps/`, `standard_fonts/`, `wasm/`, and `iccs/` asset folders. For the extension staging tree only, it remaps `build/pdf.mjs`, `build/pdf.worker.mjs`, `web/pdf_viewer.css`, and `web/pdf_viewer.mjs` to the matching legacy files from the same pinned tarball, then regenerates the packaged PDF.js manifest hashes. The repository’s normal `web/` tree remains modern.
